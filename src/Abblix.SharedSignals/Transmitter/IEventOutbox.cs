...
/// <summary>
/// Where minted SETs wait for delivery, one queue per stream - and a stream is the PAIR of a
/// receiver and an identifier, the same key <see cref="IStreamStore"/> uses. Keying on the
/// identifier alone is not a shortcut but a different model: two receivers naming their streams
/// alike then share one queue, each is handed the other's SETs, and whichever acknowledges first
/// destroys events the other never saw. The queue IS the holding the
