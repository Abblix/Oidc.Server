...
public class MvcModelGenerator : IIncrementalGenerator
{
	...
	private static StubInfo ExtractStub(GeneratorAttributeSyntaxContext context)
	{
		...
		var coreTypeName = attribute.ConstructorArguments.Length == 1 &&
		                   attribute.ConstructorArguments[0].Value is INamedTypeSymbol coreType
	...
	{
		var hintName = $"{stub.Namespace}.{stub.Name}.g.cs";
		var diagnostics = new List<DiagnosticInfo>();

		var coreType = compilation.GetTypeByMetadataName(stub.CoreTypeName);
		if (coreType == null)
		{
			diagnostics.Add(new DiagnosticInfo(CoreTypeNotFound, stub.Location, stub.CoreTypeName));
			return new GenerationResult(hintName, null, new EquatableArray<DiagnosticInfo>([.. diagnostics]));
		}

		var binderMap = BuildBinderMap(compilation);
		var properties = CollectProperties(coreType);
		...
		foreach (var property in properties)
		{
			...
			{
				EmitPropertyAttribute(
					writer, attribute, property, coreType, compilation, binderMap, diagnostics, stub.Location);
			}
		...
		writer.AppendLine("\t/// Projects this transport-bound model onto its core counterpart, copying every bound");
		writer.AppendLine("\t/// parameter so the core pipeline operates on a transport-agnostic shape.");
		writer.AppendLine("\t/// </summary>");
		writer.AppendLine($"\tpublic {coreType.ToDisplayString(FullyQualifiedWithNullability)} Map() => new()");
		writer.AppendLine("\t{");
		...
		return new GenerationResult(
			hintName, writer.ToString(), new EquatableArray<DiagnosticInfo>([.. diagnostics]));
	}

	private static void EmitPropertyAttribute(
		StringBuilder writer,
		AttributeData attribute,
		IPropertySymbol property,
		INamedTypeSymbol coreType,
		Compilation compilation,
		Dictionary<string, INamedTypeSymbol> binderMap,
		...
		LocationInfo location)
	{
		...
		switch (attributeNamespace)
		{
			...
			case DeclarativeValidationNamespace
				...
				writer.AppendLine(
					"\t[global::Microsoft.AspNetCore.Mvc.ModelBinder(" +
					$"typeof({binder.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}))]");
				return;
	...
	private static string RenderConstructorArgument(TypedConstant constant)
		...
		=> constant.Kind == TypedConstantKind.Array
			? string.Join(", ", constant.Values.Select(RenderTypedConstant))
			: RenderTypedConstant(constant);
		...
		foreach (var type in GetAllTypes(compilation.Assembly.GlobalNamespace))
		{
			foreach (var attribute in type.GetAttributes())
			{
				if (attribute.AttributeClass?.ToDisplayString() == BindsAttributeName &&
				    attribute.ConstructorArguments.Length == 1 &&
				    attribute.ConstructorArguments[0].Value is INamedTypeSymbol marker)
				{
