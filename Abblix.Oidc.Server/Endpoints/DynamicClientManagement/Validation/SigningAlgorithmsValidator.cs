...
public class SigningAlgorithmsValidator: SyncClientRegistrationContextValidator
{
    ...
    protected override ClientRegistrationValidationError? Validate(ClientRegistrationValidationContext context)
    {
        ...
        return Validate(request.RequestObjectSigningAlg, "request_object")
            ?? Validate(request.BackChannelAuthenticationRequestSigningAlg, "backchannel_authentication_request")
            ?? Validate(request.TokenEndpointAuthSigningAlg, "token_endpoint")
            ;
    }
