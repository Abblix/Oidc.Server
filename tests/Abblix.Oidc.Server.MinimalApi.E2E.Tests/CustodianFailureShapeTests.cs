...
/// <summary>
/// What a caller is told when the keys the server signs with cannot be reached. The status is the whole of the
/// answer here: no registered OAuth error code describes a server that cannot fulfil a valid request, so a client
/// decides whether to come back from the status alone, and a body carrying the exception would tell it nothing it
/// can use while telling anyone watching where the keys live.
/// </summary>
public sealed class CustodianFailureShapeTests
{
    /// <summary>
    /// Stands in for a key provider whose custodian is refusing. It throws on publication, which is the call
    /// every affected endpoint makes: the token endpoint reaches it to choose a signing key, and the JWKS
    /// endpoint to publish the public halves. The failure names the role that asked, so a test can tell which
    /// of the two enumerations the response came from.
    /// </summary>
