...
public class DistributedCacheStorageTests
{
    ...
    public async Task GetAsync_WithRemoveOnRetrievalTrue_ShouldRemoveFromCache()
    {
        ...
        // Assert - verify atomic operations occurred
        _cache.Verify(c => c.SetAsync(lockKey, It.IsAny<byte[]>(), It.IsAny<DistributedCacheEntryOptions>(), It.IsAny<CancellationToken>()), Times.Once);
        _cache.Verify(c => c.RemoveAsync(key, It.IsAny<CancellationToken>()), Times.Once);
        _cache.Verify(c => c.GetAsync(lockKey, It.IsAny<CancellationToken>()), Times.Once);
        _cache.Verify(c => c.RemoveAsync(lockKey, It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task GetAsync_WithRemoveOnRetrievalFalse_ShouldNotRemoveFromCache()
    {
        ...
        _cache.Verify(c => c.GetAsync(key, It.IsAny<CancellationToken>()), Times.Once);
        // MockBehavior.Strict ensures RemoveAsync is not called
    }
    ...
    /// <summary>
    /// Verifies that a registered take-once store decides the redemption, and the cache is not asked.
    /// </summary>
    /// <remarks>
    /// The seam is optional, so both arms ship. Only the arm that predates it was pinned, which leaves the
    /// registration reading as wired while nothing proves the value comes back through it. The strict cache
    /// mock is what proves the other arm did not run: any call to it fails the test.
    /// </remarks>
    [Fact]
    public async Task GetAsync_RemovingWithATakeOnceStoreRegistered_TakesThroughItAndNotTheCache()
    {
        var key = "test_key";
        var takenBytes = new byte[] { 9, 8, 7 };
        var expected = CreateTestData();

        var takeOnceStore = new Mock<ITakeOnceStore>(MockBehavior.Strict);
        takeOnceStore
            .Setup(store => store.TryTakeAsync(key, It.IsAny<CancellationToken>()))
            .ReturnsAsync(takenBytes);

        _serializer.Setup(s => s.Deserialize<TestData>(takenBytes)).Returns(expected);

        var storage = new DistributedCacheStorage(_cache.Object, _serializer.Object, takeOnceStore.Object);

        var result = await storage.GetAsync<TestData>(key, removeOnRetrieval: true);

        Assert.Same(expected, result);
        takeOnceStore.Verify(store => store.TryTakeAsync(key, It.IsAny<CancellationToken>()), Times.Once);
    }

    /// <summary>
    /// Verifies that a take-once store answering with nothing produces no entity, rather than a default one.
    /// </summary>
    [Fact]
    public async Task GetAsync_ATakeOnceStoreAnsweringNothing_ProducesNoEntity()
    {
        var key = "test_key";
        ...
        Assert.Null(await storage.GetAsync<TestData>(key, removeOnRetrieval: true));
    }

    /// <summary>
    /// Verifies that a registered take-once store is left out of a read that does not redeem.
    /// </summary>
    /// <remarks>
    /// Taking is destructive, so routing an ordinary read through it would consume the entity the caller
    /// only wanted to look at. Both mocks are strict, so each names the other arm's mistake.
    /// </remarks>
    [Fact]
    public async Task GetAsync_ReadingWithoutRemoving_LeavesTheTakeOnceStoreAlone()
    {
        var key = "test_key";
        ...
        _cache.Setup(c => c.GetAsync(key, It.IsAny<CancellationToken>())).ReturnsAsync(cachedBytes);
        _serializer.Setup(s => s.Deserialize<TestData>(cachedBytes)).Returns(expected);

        var storage = new DistributedCacheStorage(_cache.Object, _serializer.Object, takeOnceStore.Object);

        Assert.Same(expected, await storage.GetAsync<TestData>(key, removeOnRetrieval: false));
        takeOnceStore.VerifyNoOtherCalls();
    }
}
