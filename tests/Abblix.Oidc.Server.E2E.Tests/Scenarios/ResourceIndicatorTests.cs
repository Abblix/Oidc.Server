...
public class ResourceIndicatorTests(TestFactory factory) : TestBase(factory)
{
    ...
    /// <summary>
    /// RFC 7662 is written for this caller. Section 2.1 has the protected resource call the endpoint, and
    /// Section 4 says what the authorization server owes it: "determine whether or not the token can be used
    /// at the resource server making the introspection call." The resource server is not the client the token
    /// was issued to, so ownership alone cannot be the rule.
    /// </summary>
    [Fact]
    public async Task Resource_server_may_introspect_a_token_issued_for_it_to_another_client()
    {
        var client = CreateClient();
        var discovery = await FetchDiscoveryAsync(client);

        var accessToken = await ObtainResourceScopedClientCredentialsTokenAsync(client, discovery);

        var body = await IntrospectAsync(client, discovery, accessToken, TestConstants.ConfidentialClientId);
        Assert.True(
            body[IntrospectionSuccess.Parameters.Active]!.GetValue<bool>(),
            $"the resource server was told its own token does not exist: {body.ToJsonString()}");
    }

    /// <summary>
    /// The other half: naming a resource does not open its tokens to every authenticated client. RFC 7662
    /// Section 2.2 reserves active:false for exactly this - "the protected resource is not allowed to
    /// introspect this particular token".
    /// </summary>
    [Fact]
    public async Task A_client_not_acting_for_the_resource_may_not_introspect_its_tokens()
    {
        var client = CreateClient();
        var discovery = await FetchDiscoveryAsync(client);
        ...
        Assert.False(
            body[IntrospectionSuccess.Parameters.Active]!.GetValue<bool>(),
            $"an unrelated client read a token issued for {TestConstants.ApiResource}: {body.ToJsonString()}");
    }

    private static async Task<string> ObtainResourceScopedClientCredentialsTokenAsync(
