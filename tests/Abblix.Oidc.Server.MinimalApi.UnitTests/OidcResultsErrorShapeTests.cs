...
/// </summary>
/// <remarks>
/// The twin of the MVC suite's <c>ActionResultErrorShapeTests</c>, case for case and assertion for assertion.
/// The two adapters are meant to answer identically, so the cases are deliberately not divided between them:
/// what a difference in coverage hides is precisely the arm where one adapter answers 403 and the other 401.
///
...
/// </remarks>
public class OidcResultsErrorShapeTests
{
    private const string Realm = "https://auth.example.com";
    private static readonly string[] DPoPAlgs = ["RS256", "ES256"];

    private static string Challenge(HttpResultRunner.Response response)
        ...
        string errorCode, int fallbackStatusCode)
    {
        ...
        var response = await HttpResultRunner.RunAsync(error.Format(fallbackStatusCode, Realm));
